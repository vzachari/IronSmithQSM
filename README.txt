...
1) Software requirements:
	...
	a) Ironsmith requires Matlab to run MEDI and supports versions R2017b to R2019b. Matlab is not needed if MEDI is not required.
...
3) MyInputFile format:  
	...
	### If MEDI is required to create QSM images/maps:  
		...
		Column3 = Absolute path to directory with MPRAGE/MEMPRAGE files (e.g. /home/subjecs/S01/MPR). MPR/MEMPR files can be:  
			...
			a) DICOMS (Only MPR/MEMPR DICOMS should be present in the MPR folder if you want IronSmith to process the DICOMS. 
...
4) Quality of life features:  
	...
	and you would like Ironsmith to skip the segmentation step, do the following: 

		a) copy the recon-all folder (the one containing the label, mri, scripts, stats, surf ... folders) into 
			/OutputFolder/Freesurfer, where OutputFolder is the one specified in the Ironsmith command syntax
			You can create the /OutputFolder/Freesurfer folder or Ironsmith will create it for you 
			if you have run it at least once previsouly
 
		b) Rename the recon-all folder to Subj_Freesurfer_Skull. Subj should match the one provided in MyInputFile 
...
5) Outputs
	...
	and "S0001" is one of the participants processed, then /home/QSM_Analysis/S0001 will be created and populated with data:
		...
		b) All QSM maps/images created are under S0001/QSM/Freesurf_QSM_Masks and are labelled as Subj_QSM_Map_FSL.nii.gz
		...
		d) QSM per ROI means (87 ROIs) are under /QSM_Analysis/Group as follows:
			...
			Group_QSM_SNR.csv <--- Per ROI SNR measures: Per ROI, mean intensity of magnitude image within ROI / Standard deviation of magnitude signal outside the head
